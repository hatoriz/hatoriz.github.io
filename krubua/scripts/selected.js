...
Selected.prototype = {
    ...
    init: function() {
        ...
        // Hatoriz
        // Temporary disabling the key play
        ...
        // Hatoriz 
        // Disable automatic song play here
        ...
        this.audio.ontimeupdate = function(e) {
            ...
            for (var i = 0, l = that.lyric.length; i < l; i++) {
                if (this.currentTime > that.lyric[i][0] - 0.50 /*preload the lyric by 0.50s*/ ) {
										// Make the delay by pausing the audio and make it run again
										// after the timeout. The logic also check all necessary value
										// to prevent timeout running multiple times.
										if (i > 0 && triggerredDelayIndex < i) {
												triggerredDelayIndex = i;
												isDelay = true;
												that.audio.pause();
												setTimeout(function(){
													that.audio.play();
										...
										// Run these codes when not still in delay.
										if (!isDelay){
