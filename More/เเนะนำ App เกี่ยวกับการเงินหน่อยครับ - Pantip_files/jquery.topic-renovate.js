...
(function($){	
	...
	$.commentTopic.replyPrev = function(){
		$(document).on('click','.load-reply-prev',function(){
			...
			//$('#'+refBar).find('.focus-txt').text('กำลังโหลดข้อมูล...').end().after('<div data-refcm="comment-'+refId+'" class="comment-'+refId+' ref" id="reply-sub-st-'+unix+'"></div>');
	...
	$.commentTopic.replyNext = function(){
		$(document).on('click','.load-reply-next',function(){
			...
			//$('#'+refBar).find('.focus-txt').text('กำลังโหลดข้อมูล...').end().after('<div data-refcm="comment-'+refId+'" class="comment-'+refId+' ref" id="reply-sub-ed-'+unix+'"></div>');
	...
	$.commentTopic.showReply = function(){ 
		$(document).on('click','.reply.see-more',function() {
			//var root = $(this).parents('.section-comment');
			//var root = $(this).prev();
	...
	$.commentTopic.getComment = function (){
		...
		$.ajax({
			...
			//				/*for new hide reply ๛*/
			//				var taget = "";
			//				var type = param.match(/^(comment|page)/i);
			//				if(type == null)
			//				{
			//					return false;
			//				}
			//				if(type[0] == 'page')
			//				{
			//					taget  = param.match(/^((page)([0-9]+))/i);
			//
			//				}else if(type[0] == 'comment'){
			//
			//					taget  = param.match(/^((comment)([0-9\-]+))/i);
			//				}
			//				
			//				// check if taget has reply no. eg. 10-6
			//				var position = taget[3].split('-');
			//				if(position.length == 2)
			//				{
			//					
			//					// calculate steping 100
			//					var item_per_page = 100;
			//					var item_id = position[0]-1;
			//					var power = Math.floor(item_id/item_per_page)
			//					
			//					// re calculate item_id
			//					item_id = item_id - (power*item_per_page);
			//					var taget_id = rs.comments[item_id]['_id'];
			//
			//					// show that position
			//					var that = $('.comment-'+taget_id);
			//					that.show();
			//
			//					// show top and bottom loadmore (if)
			//					that.prev('.loadmore-bar.firsttop').show();
			//					that.next('.loadmore-bar.bottom').show();
			//					
			//				}
			...
			//				$('.comment-action').on('click','.icon-heart-like,.icon-heart-dislike',$.comment_vote.init);
	...
	function scrollTopic(p)
	{
		...
		{
			//			console.log(p);
			//			var re = /(comment)([0-9\-]+)/i;
			//			var str = p[3];
			//			var newurl = str.match(re);
		...
		//			else
		//			{
		//				//var targetOffset = $('.ads-leaderboard').offset().top;
		//				$('html,body').animate({
		//					scrollTop: 98
		//				}, 100);
		//			}
			...
			$.ajax({
				...
				success : function(rs){
					...
					if(rs.status == 'success')
					{
						...
						/*api_push_call($.extend(dataApi,rs), function() {
							var msg = 'ส่งข้อความสำเร็จแล้ว, <a href="javascript:void(0);" id="go">ดูข้อความของคุณ</a><br/><span class="small-txt">ไปยังข้อความของคุณ';
							lightboxSave(msg,rs.topic_id , rs.comment_no);	
						});*/
	...
	function lightboxSave(msg,topic_id,comment_no)
	{
		...
		//		});
		//
		//		$('#lightbox_save').html('<div class="modal-success-wrapper"><div class="icon icon-success modal-success"></div>'+msg+' ภายใน <span id="timer"></span> วินาที</span></div>');
		//		/* click */
		//		$('#go').click(function(){
		//			if(comment_no == null)
		//			{
	...
	$.oauthFacebook.btnLogin = function()
	{
		...
		//		$(document).on('keydown', '#member_password', function(e){
		//			if(e.which == 13)
		//			{
		//				$('#user_login_btn_fb').trigger('click');
		//			}
		//		});
	...
	function form_fb_login($obj)
	{
		...
		{
			//			alert('หน้าจอแสดง ใส่รหัสผ่านเพื่อ login');
			//			$('.login_lb_process').dialog('close');
	...
	//		console.log(raw_id);
	//		return false;
	//		var normal_comment_id = 	parseInt(raw_id.replace('comment-',''));
	//		// ถ้าไม่ใช่ comment ธรรมดาก็อาจจะเป็น best comment / comment ที่ถูกเลือกโดย owner topic
	//		if(!normal_comment_id)
	//		{
	//			var best_comment_id  = 	parseInt(raw_id.replace('best-answer-',''));
	//
	//			if(best_comment_id > 0)
	//			{
	//				is_best_comment = 'best_comment';
	//				return best_comment_id;
	//			}
	//			else
	//			{
	//				var best_comment_by_owner_topic_id = parseInt(raw_id.replace('best-answer-by-owner-topic-',''));
	//				is_best_comment = 'best_comment_by_owner';
	//				return best_comment_by_owner_topic_id;
	//			}
	//		}
	//		is_best_comment = 'normal';
	//		return normal_comment_id
	}
	...
	{
		//		var chk_ele_normal;
		//		var chk_ele_best_comment;
		//		var chk_ele_best_owner;
	...
	//		//ปิดกล่อง  comment ของ thread
	//		$(document).on('click', '.close-comment-box', function(){
	//			delete_tabindex();
	//			$('.sub-comment.reply-comment').remove();
	//		});
	};
	...
	//ส่ง Passcode อีกครั้ง
	/** ปิด ส่ง Pass Code อีกครั้งก่อน
		if(ajax_Sending == false)
		{
			$('#lb_process').on('click', '#pc_send_new', function(){
				ajax_Sending = true;
				$.ajax({
					type: "POST",
					url: '/login/ajax_send_passcode_for_email',
					cache: false,
					data :'input='+pinputUser+'&type='+ptype,
					dataType : 'json',
					success: function(result){
						if(result.chk == 'true')
						{
	...
	$.oauthSetPwd.confirm = function()
	{
		...
		$(document).on('click', '#oauth_pwd_btn', function(){
			...
			{
				//				$('#oauth_pwd_btn').after('<span class="loading-inline focus-txt oauth-loading"> กำลังโหลดข้อมูล โปรดรอสักครู่</span>');
				//console.log($(document.body).data('oauth_login'));
	...
	/* Oauth Hotmail */
	//	var APP_CLIENT_ID = '00000000440C6295';
	//	var REDIRECT_URL = '';
	//	WL.Event.subscribe("auth.login", onHotmailLogin);
	//	
	//	WL.init({
	...
	//
	//	$.oauthHotmail = {};
	//	$.oauthHotmail.authen = function (){
	//		$(document).on('click','#ht_authen',function(){
	//			signInUserWl();
	//		});
	//	};
	...
	$.emotion.listen = function (){
		...
		$(document).on('click','.display-post-emotion', function(event){
			...
			//$(this).parents('.display-post-story-footer ').find('.emotion-vote-list').children('.emotion-vote-choice').slideToggle();
			//$(this).parents('.display-post-story-footer ').find('.emotion-vote-list').slideToggle();
	...
	function add_polls_result()
	{
		...
		//	console.log($.data(document.body).polls_result);return false;
		//		if(permission_vote_poll(topic) == true)
		//		{
		//			console.log('ttttt');
		//		}
		//		console.log('gfdgfdgdf');
		//		return false;
	...
	function check_count_answer(obj, cls, que, type)
	{
		...
		if(cls == "qtype_6")
		{
			...
			//set_json(selected, queue, question_id, poll_type);
		...
		else if(cls == "qtype_5")
		{
			...
			obj.find('.grid-radio-rows').each(function(index) {
				...
				if($(this).find('.radio-choice:checked').length > 0)
				{
					...
					//					checked['result'][index] = $(this).find('.radio-choice:checked').val();
			...
			//set_json(checked, queue, question_id, poll_type);
			//console.log(checked);
			checked['cnt'] = cnt;
			return checked;
		}
		...
		{
			//console.log(obj.find('.selectbox').val());
			...
			//			checked['result'][0] = obj.find('.selectbox').val();
		...
		{
			//console.log(obj.find('.checkbox:checked').val());
			
			var cnt_radio = obj.find('.checkbox-choice').length;
			//			var val_checked = obj.find('.checkbox-choice:checked').val();
			//			var split_checked = val_checked.split('_');
			...
			//				return false;
			//			}
			//console.log(obj.find('.checkbox-choice:checked'));return false;
			//console.log(obj.find('.checkbox-choice:checked').length);return false;
			var cnt = 1;
		...
		{
			//console.log(obj.find('.radio:checked').val());
			var cnt = 1;
			...
			else
			{
				...
				var split_checked = val_checked.split('_');
				//				console.log(split_checked.length, cls);return false;
				if(split_checked.length > 1 && split_checked[2] >= cnt_radio)
				{
					msgErrorPolls();
				}
	...
	$.poll.editVote = function (){
		$(document).on('click', '#btn_edit_vote', function(){
			var pathArray = window.location.pathname.split( '/' );
			var topic = pathArray[2];
			window.location = 'http://'+window.location.hostname+'/topic/'+topic+'/edit';
		});
	};
	...
	$.poll.resultVote = function (){
		$(document).on('click', '#btn_vote_result', function(){
			var pathArray = window.location.pathname.split( '/' );
			var topic = pathArray[2];
			var url = 'http://'+window.location.hostname+'/topic/'+topic+'/result';
			window.open(url)
		});
	};
