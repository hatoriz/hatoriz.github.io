...
function th_ajax_tracker(pagename){ if(!truehitsurl || !truehitsurl_top || !hash) return; if((pagename!=null)&&(pagename.replace != null)) pagename=pagename.replace(/%/g,"%25").replace(/ /g,"%20").replace(/\"/g,"%22").replace(/#/g,"%23").replace(/&/g,"%26").replace(/\'/g,"%27").replace(/\+/g,"%2b").replace(/=/g,"%3d").replace(/\</g,"%3c").replace(/\>/g,"%3e").replace(/\:/g,"%3a"); else __th_page = '';if((domain_cmp(document.domain,turlnameindex))&&(domain_cmp(truehitsurl_top,turlnameindex))){rf='ajax';if(Tracker){ _narg = Tracker(_hc,turlnameindex); }if(VisitorT==null) VisitorT=0;var _th_rand=Math.random(); arg="&rand="+_th_rand+"&bv="+VisitorT+"&rf="+rf+"&web="+hash+collector()+_narg+"&truehitspage="+pagename+"&truehitsurl="+truehitsurl;var _i=new Image();_i.src='http://'+_hsv+'/'+_ht+'?hc='+_hc+arg;_i.onload=function(){ return; };arg="";}}
...
if( th_img[0] != null ){
th_img[0].src = 'http://'+_hsv+'/'+_ht+'?hc='+_hc+arg + "&async=1";
th_img[0].alt = 'Thailand Web Stat';
th_img[0].border = 0;
}
var th_link = document.getElementsByName('th_link');
if( th_link[0] != null ){
th_link[0].href = 'http://truehits.net/'+_ctg;
}
arg="";
}
